...
public class PSTParser implements Parser {
    ...
    public TreeItem<TreeObject> parseFile(File file, ParserConfiguration configuration, int totalFiles) {
        ...
        Task task = new Task() {
            ...
            protected CheckBoxTreeItem<TreeObject> call() throws Exception {
                ...
                if (pstFile.getRootFolder() != null) {
                    ...
                    for (PSTFolder subFolder : pstFile.getRootFolder().getSubFolders()) {
                        processFolder(subFolder, pstFile, rootTreeItem);
                    }
                }
                return rootTreeItem;
            }
             ...
             */
            private void processFolder(PSTFolder folder, PSTFile pstFile, CheckBoxTreeItem<TreeObject> treeItem) {
                try {
                    ...
                    if (!isParsed && folder.getContentCount() > 0) {
                        ...
                        while ((pstObject = folder.getNextChild()) != null) {
                            ...
                            if (pstObject instanceof PSTContact) {
                                ...
                                indexer.index(message, pstFile, folder);
                            }
                    ...
                    if (folder.getNodeType() != 3) {
                        for (PSTFolder subFolder : folder.getSubFolders()) {
                            processFolder(subFolder, pstFile, folderTreeItem);
                        }
                    } else {
