...
public class PSTMessageFactory implements MessageFactory {
     ...
     * <p>
     * Static since we don't want a reference to this class in the PSTParser class.
    ...
    public PSTMessageFactory(Case currentCase, BookmarksModel bookmarksModel, TagsDAO tagsDAO) {
    ...
    public MessageRow getMessageRow(String id) {
        try {
            ...
            if (pstObject instanceof PSTContact) {
                ...
                messageID = IDGenerator.getID(contact, pstFile);
                ...
                messageID = IDGenerator.getID(appointment, pstFile);
                ...
                messageID = IDGenerator.getID(message, pstFile);
            ...
            SimpleStringProperty tagProperty = new SimpleStringProperty(tagsDAO.getTag(messageID));
            ...
            tagProperty.addListener((ObservableValue<? extends String> observable, String oldValue, String newValue) -> {
                if (!newValue.equals(oldValue)) {
                    ...
                    tagsDAO.setTag(messageRow.getId(), newValue);
                }
            });
    ...
    public MessageValue getMessageValue(String id) {
        try {
            ...
            final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat(Settings.get(currentCase.getName(), "DateFormat"));
            ...
            if (pstObject instanceof PSTContact) {
                ...
                try {
                    ...
                    log.debug("HTML body: {}", contact.getBodyHTML());
                    log.debug("Plaintext body: {}", contact.getBody());
                    log.debug("-------------------------------------");
                }
                ...
                try {
                    ...
                    log.debug("-------------------------------------");
                    log.debug("Possibly fallback?");
                    log.debug("HTML body: {}", appointment.getBodyHTML());
                    log.debug("Plaintext body: {}", appointment.getBody());
                    log.debug("-------------------------------------");
                }
    ...
    public List<MessageRow> getMessagesFromTreeItem(TreeItem<TreeObject> treeItem) {
        ...
        try {
            ...
            // Each PST message is indexed with a "FolderID" field, searching
            // for this field will return every message in a specific folder.
            List<Document> results = Searcher.getInstance(currentCase.getName()).search(
                    new TermQuery(new Term("FolderID", treeItem.getValue().getFolderID())),
                    Integer.MAX_VALUE
            );
